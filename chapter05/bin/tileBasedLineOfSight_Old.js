let thingsToLoad = ["images/monsterMaze.png", "maps/monsterMaze.json"];
...
//Game variables
let world, alien, message, wallMapArray, bombMapArray, bombSprites, bombLayer, leftArrow, upArrow, downArrow, rightArrow;
...
function play() {
  ...
  monsters.forEach(monster => {
    ...
    if (isCenteredOverCell(monster)) {
      ...
      monster.direction = findDirection(monster, monster.direction, alien, wallMapArray, world);
    ...
    //state to `normal`
    monster.lineOfSight = tileBasedLineOfSight(monster, alien, wallMapArray, world, 0, [90, -90, 0, 180, -180]);
...
function isCenteredOverCell(sprite) {
  let trueOrFalse = Math.floor(sprite.x) % world.tilewidth === 0 && Math.floor(sprite.y) % world.tileheight === 0;
  return trueOrFalse;
}
...
//restrict the line of sight
  ...
  let points = () => {
    ...
    for (let i = 1; i <= numberOfPoints; i++) {
      ...
      let index = g.getIndex(x, y, world.tilewidth, world.tileheight, world.widthInTiles);
      let point = {
        x: x,
        y: y,
        index: index
...
function findDirection(spriteOne, spriteOneDirection, spriteTwo, mapArray, world) {
  ...
  let hasLineOfSight = tileBasedLineOfSight(spriteOne, //The first sprite
...
//# sourceMappingURL=tileBasedLineOfSight_Old.js.map