...
let thingsToLoad = ["images/cubes.png", "images/cubes.json"];
...
let world, leftArrow, upArrow, rightArrow, downArrow, message, wallLayer, player, groundLayer, wallMapArray;
...
function setup() {
  ...
  world = makeIsoTiledWorld("images/cubes.json", "images/cubes.png");
...
function hitTestIsoTile(sprite, mapArray, gidToCheck, world, pointsToCheck) {
  ...
  let checkPoints = key => {
    ...
    if (world.cartTilewidth && world.cartTileheight) {
      ...
      collision.index = g.getIndex(point.x, point.y, world.tilewidth, world.tileheight, world.widthInTiles);
    }
